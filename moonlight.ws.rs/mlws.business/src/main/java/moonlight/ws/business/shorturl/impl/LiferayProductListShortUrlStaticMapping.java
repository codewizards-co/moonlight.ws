...
/**
 * Static mapping of short-URL for Liferay-product-list.
 * <p>
 * <ul>
 * <li>Short URL:
 * <code>{{{@link ShortUrlConfig#URL SHORTURL_URL}}}/p</code></li>
 * <li>Real URL:
 * <code>{{{@link LiferayConfig#URL LIFERAY_URL}}}/web/dkc/products</code></li>
 * </ul>
 */
@ApplicationScoped
public class LiferayProductListShortUrlStaticMapping extends RegexShortUrlStaticMapping
	...
	protected Pattern createLongUrlPattern() {
		...
		return Pattern.compile(Pattern.quote(liferayUrl) + "(?:\\/[a-z]{2})?\\/web\\/dkc\\/products(\\/?)");
	}

	@Override
	protected String getLongUrl(@NonNull String code) {
		return concatUrlSegments(liferayConfig.getUrl(), "web/dkc/products", code);
	}
